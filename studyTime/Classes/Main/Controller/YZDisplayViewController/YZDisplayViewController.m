...
- (UIView *)separatorV{
    if (_separatorV == nil) {
        ...
        separatorV.backgroundColor = _separatorViewColor?_separatorViewColor:STLightLogoColor;
        [self.titleScrollView addSubview:separatorV];
        _separatorV = separatorV;
    }
    return _separatorV;
}
...
- (void)viewDidLayoutSubviews
{
    ...
    if (self.contentView.height == 0) {
        // 没有设置内容尺寸，才需要设置内容尺寸
        CGRect frame = CGRectMake(0, contentY, contentW, contentH);
        if (!self.navigationController.navigationBar.isTranslucent) {
            frame = CGRectMake(0, 0, contentW, contentH);
        }
    ...
    self.separatorV.frame = CGRectMake(0, titleH - 0.5, contentW, 0.5);
    // 设置内容滚动视图frame
